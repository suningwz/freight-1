odoo.define('freight_attendance.greeting_message', function (require) {
    "use strict";
    ...
    var GreetingMessage = AbstractAction.extend({
        template: 'FreightAttendanceGreetingMessage',
    
        events: {
            "click .o_freight_attendance_button_dismiss": function() { this.do_action(this.next_action, {clear_breadcrumbs: true}); },
        },
        ...
        init: function(parent, action) {
            ...
            if(!action.attendance) {
                ...
                this.getSession().user_has_group('freight.group_freight_attendance_user').then(function(has_group) {
                    if(has_group) {
                        self.next_action = 'freight.freight_attendance_action_kiosk_mode';
                    } else {
                        self.next_action = 'freight.freight_attendance_action_my_attendances';
                    }
                });
                return;
            }
    
            this.next_action = action.next_action || 'freight.freight_attendance_action_my_attendances';
            // no listening to barcode scans if we aren't coming from the kiosk mode (and thus not going back to it with next_action)
            if (this.next_action != 'freight.freight_attendance_action_kiosk_mode' && this.next_action.tag != 'freight_attendance_kiosk_mode') {
                this.activeBarcode = false;
            }
        ...
        welcome_message: function() {
            ...
            if (now.hours() < 5) {
                this.$('.o_freight_attendance_message_message').append(_t("Good night"));
            } else if (now.hours() < 12) {
                if (now.hours() < 8 && Math.random() < 0.3) {
                    if (Math.random() < 0.75) {
                        this.$('.o_freight_attendance_message_message').append(_t("The early bird catches the worm"));
                    } else {
                        this.$('.o_freight_attendance_message_message').append(_t("First come, first served"));
                    }
                } else {
                    this.$('.o_freight_attendance_message_message').append(_t("Good morning"));
                }
            } else if (now.hours() < 17){
                this.$('.o_freight_attendance_message_message').append(_t("Good afternoon"));
            } else if (now.hours() < 23){
                this.$('.o_freight_attendance_message_message').append(_t("Good evening"));
            } else {
                this.$('.o_freight_attendance_message_message').append(_t("Good night"));
            }
            if(this.previous_attendance_change_date){
                ...
                if(now - last_check_out_date > 24*7*60*60*1000){
                    this.$('.o_freight_attendance_random_message').html(_t("Glad to have you back, it's been a while!"));
                } else {
                    if(Math.random() < 0.02){
                        this.$('.o_freight_attendance_random_message').html(_t("If a job is worth doing, it is worth doing well!"));
                    }
                }
            }
        },
        ...
        farewell_message: function() {
            ...
            if(this.previous_attendance_change_date){
                ...
                if(now - last_check_in_date > 1000*60*60*12){
                    this.$('.o_freight_attendance_warning_message').show().append(_t("<b>Warning! Last check in was over 12 hours ago.</b><br/>If this isn't right, please contact Human Resource staff"));
                    clearTimeout(this.return_to_main_menu);
                    this.activeBarcode = false;
                } else if(now - last_check_in_date > 1000*60*60*8){
                    this.$('.o_freight_attendance_random_message').html(_t("Another good day's work! See you soon!"));
                }
            }
    
            if (now.hours() < 12) {
                this.$('.o_freight_attendance_message_message').append(_t("Have a good day!"));
            } else if (now.hours() < 14) {
                this.$('.o_freight_attendance_message_message').append(_t("Have a nice lunch!"));
                if (Math.random() < 0.05) {
                    this.$('.o_freight_attendance_random_message').html(_t("Eat breakfast as a king, lunch as a merchant and supper as a beggar"));
                } else if (Math.random() < 0.06) {
                    this.$('.o_freight_attendance_random_message').html(_t("An apple a day keeps the doctor away"));
                }
            } else if (now.hours() < 17) {
                this.$('.o_freight_attendance_message_message').append(_t("Have a good afternoon"));
            } else {
                if (now.hours() < 18 && Math.random() < 0.2) {
                    this.$('.o_freight_attendance_message_message').append(_t("Early to bed and early to rise, makes a man healthy, wealthy and wise"));
                } else {
                    this.$('.o_freight_attendance_message_message').append(_t("Have a good evening"));
                }
            }
        },
    ...
    core.action_registry.add('freight_attendance_greeting_message', GreetingMessage);
